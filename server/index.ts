...
// ============= 私人财务管家 · 人格 =============
const FINANCE_SYSTEM_PROMPT = `你是用户的私人财务管家，只认数字，不认借口。
...
app.get("/api/ledger", (req, res) => {
  try {
    ...
    res.status(500).json({ error: error?.message || "读取账本失败" });
  }
});

app.put("/api/ledger", (req, res) => {
  try {
    const { budget, mustSave, items } = req.body || {};
    const payload = {
    ...
    res.status(500).json({ error: error?.message || "保存账本失败" });
  }
});
...
app.post("/api/chat", async (req, res) => {
  const { sessionId, message, model, systemPrompt, cwd, permissionMode, context } = req.body;
  ...
  // 默认系统提示词
  const defaultSystemPrompt = FINANCE_SYSTEM_PROMPT;
  const effectiveSystemPrompt = context ? `${systemPrompt || defaultSystemPrompt}

${context}` : (systemPrompt || defaultSystemPrompt);
  ...
  try {
    ...
    for await (const msg of stream) {
      ...
      if (msg.type === "system" && (msg as any).subtype === "init") {
        ...
        res.write(`data: ${JSON.stringify({ 
          type: "done", 
          duration: resultMsg.duration ?? resultMsg.duration_ms, 
          cost: resultMsg.cost ?? resultMsg.total_cost_usd 
        })}\n\n`);
      }
    }
